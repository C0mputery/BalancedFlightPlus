...
public class PlacementDisruptorBehaviour extends BlockEntityBehaviour
{
    public static final BehaviourType<PlacementDisruptorBehaviour> TYPE = new BehaviourType();

    public PlacementDisruptorBehaviour(SmartBlockEntity be)
    {
        super(be);
    }
    ...
    public void setActive(boolean active)
    {
        ...
        if (!active)
        {
            PlacementDisruptorEntity.ActiveDisruptors.remove(getPos());
            playSound(getWorld(), getPos(), SoundEvents.BEACON_DEACTIVATE);
            return;
        }

        if (PlacementDisruptorEntity.ActiveDisruptors.containsKey(getPos()))
            return;

        PlacementDisruptorEntity.ActiveDisruptors.put(getPos(), (PlacementDisruptorEntity) blockEntity);
    ...
    public void initialize()
    {
        if (getWorld().isClientSide)
            ((PlacementDisruptorEntity)blockEntity).placedRenderTime = AnimationTickHolder.getRenderTime(getWorld());
    }
    ...
    public void tick() {
        ...
        var flightAnchor = (PlacementDisruptorEntity) blockEntity;
    ...
    public static void beaconTick(Level level, BlockPos blockPos, PlacementDisruptorEntity entity) {
        if (level instanceof PonderWorld ponderWorld) {
            if (!entity.beamSections.isEmpty())
                return;

            entity.beamSections.add(new BeaconBlockEntity.BeaconBeamSection(DyeColor.LIME.getTextureDiffuseColors()));
            return;
        }
